...
class Piwik
{
    ...
    {
        if( empty( $this->rangeEnd ) ) return $this->rangeStart;
        else return $this->rangeStart . ',' . $this->rangeEnd;
    }
    ...
    private function _request( $method, $params = array() )
    {
        ...
        if( !empty( $buffer ) ) $request = $this->_parseRequest( $buffer );
        else $request = false;
